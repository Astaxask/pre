...
async function main(): Promise<void> {
  ...
  const lancedbPath =
    process.env["PRE_LANCEDB_PATH"] ?? join(dataDir, "lancedb");
  ...
  if (sidecarScript) {
    try {
      ...
      if (ready) {
        ...
        console.warn(
          "[PRE Gateway] Sidecar not ready within 10s, continuing without it",
        );
      }
    } catch (e) {
  ...
  console.log(`[PRE Gateway] ${adapters.size} adapter(s) initialized`);
  ...
  bus.on("sync-completed", (payload) => {
    ...
    enqueueInferenceJob("post-sync").catch((err) => {
      const message = err instanceof Error ? err.message : String(err);
      console.error(
        `[PRE Gateway] Failed to enqueue post-sync inference: ${message}`,
      );
    });
  });
